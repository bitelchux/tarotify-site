...
"use strict";var tarotifyApp=angular.module("tarotifyApp",["ngRoute","ngAnimate","tarotifyApp.filters","tarotifyApp.services","tarotifyApp.directives","tarotifyApp.controllers"]).config(["$routeProvider",function($routeProvider){$routeProvider.when("/",{templateUrl:"partials/home.html",controller:"HomeCtrl"}).when("/about",{templateUrl:"partials/about.html",controller:"AboutCtrl"}).when("/card/Major-Arcana/:card/:reversed?",{templateUrl:"partials/card_info.html",controller:"CardInfoCtrl"}).when("/card/Minor-Arcana/:suite/:card/:reversed?",{templateUrl:"partials/card_info.html",controller:"CardInfoCtrl"}).when("/settings",{templateUrl:"partials/settings.html",controller:"SettingsCtrl"}).when("/help",{templateUrl:"partials/help.html",controller:"HelpCtrl"}).when("/new-reading",{templateUrl:"partials/new.html",controller:"NewCtrl"}).otherwise({redirectTo:"/"})}]),tarotifyAppFilters=angular.module("tarotifyApp.filters",[]),tarotifyAppServices=angular.module("tarotifyApp.services",[]),tarotifyAppDirectives=angular.module("tarotifyApp.directives",[]),tarotifyAppControllers=angular.module("tarotifyApp.controllers",[]);tarotifyAppControllers.controller("AppCtrl",["$rootScope","SettingsService",function($rootScope,SettingsService){$rootScope.themeBg=SettingsService.getBackground().class,$rootScope.position_description=null}]);
"use strict";tarotifyAppControllers.controller("CardInfoCtrl",["$scope","$location","$sce","$routeParams","TarotService",function($scope,$location,$sce,$routeParams,TarotService){$scope.card={},$scope.isReversed=!1,TarotService.initCards().then(function(){var route_path=$location.path();route_path=route_path.replace("/card/",""),route_path=route_path.replace("/reversed",""),$scope.card=TarotService.getCardByUrl(route_path),"reversed"===$routeParams.reversed&&($scope.isReversed=!0),$scope.card.meaning=$sce.trustAsHtml(String($scope.card.meaning)),$scope.card.meaning_reversed=$sce.trustAsHtml(String($scope.card.meaning_reversed)),$scope.card.description=$sce.trustAsHtml(String($scope.card.description)),$scope.card.description_source=$sce.trustAsHtml(String($scope.card.description_source)),console.log("Got card data: ",$scope.card)})}]);
"use strict";tarotifyAppControllers.controller("HelpCtrl",["$scope",function(){}]);
"use strict";tarotifyAppControllers.controller("HomeCtrl",["$scope","$rootScope","$location","$timeout","TarotService","LayoutManagerService","SettingsService",function($scope,$rootScope,$location,$timeout,TarotService,LayoutManagerService,SettingsService){var default_layout="Past, Present, Future";$scope.cards=TarotService.getCurrentCards(),$scope.layoutData=LayoutManagerService.getCurrentLayout(),$scope.showReversals=SettingsService.getAllowReversals(),TarotService.isNewGame()&&(console.log("NEW READING"),$scope.cards=[],LayoutManagerService.resetCurrentLayout(),$scope.layoutData=LayoutManagerService.getCurrentLayout(),TarotService.initCards().then(function(){TarotService.shuffleCards(),TarotService.addReversals(),$scope.cards=TarotService.getCurrentCards()}),LayoutManagerService.initLayouts().then(function(){null===$scope.layoutData&&($scope.layoutData=LayoutManagerService.setLayout(default_layout))}),$rootScope.$broadcast("NEW_READING"),TarotService.setIsNewGame(!1)),$scope.$on("CARDS_SHUFFLED",function(){$scope.cards=TarotService.getCurrentCards()}),$scope.$on("DREW_CARD",function(event,card){console.log("Drew card -- ",card),$scope.cards=TarotService.getCurrentCards();for(var num_grid_items=$scope.layoutData.grid_items.length,next_num=9999,i=0;num_grid_items>i;i++){var gridItem=$scope.layoutData.grid_items[i];gridItem.hasOwnProperty("card")||gridItem.num<next_num&&(next_num=gridItem.num)}console.log(next_num),9999!==next_num&&(LayoutManagerService.setCardOnGridItem(card,next_num),$scope.layoutData=LayoutManagerService.getCurrentLayout(),$timeout(function(){$(".tarot-card-"+next_num).addClass("fade-in")},5))}),$scope.viewCard=function(infoUrl,reversed,position_description){$rootScope.position_description=position_description,$location.path(reversed&&$scope.showReversals?"card/"+infoUrl+"/reversed":"card/"+infoUrl)}}]);
"use strict";tarotifyAppControllers.controller("NewCtrl",["$scope","$location","TarotService","LayoutManagerService",function($scope,$location,TarotService,LayoutManagerService){LayoutManagerService.initLayouts().then(function(){$scope.layouts=LayoutManagerService.getAllLayouts()}),$scope.selectLayout=function(layoutName){var startNewReading=function(){LayoutManagerService.setLayout(layoutName),TarotService.setIsNewGame(!0),$location.path("/")};LayoutManagerService.getTotalAvailablePositions()===LayoutManagerService.getTotalPositions()?startNewReading():confirm("Starting a new reading will clear all cards. Are you sure?")&&startNewReading()}}]);
"use strict";tarotifyAppControllers.controller("SettingsCtrl",["$scope","$rootScope","SettingsService","TarotService",function($scope,$rootScope,SettingsService,TarotService){$scope.backgrounds=SettingsService.getBackgrounds(),$scope.opt_showReversals=SettingsService.getAllowReversals(),$scope.opt_majorArcanaOnly=SettingsService.getMajorArcanaOnly(),$scope.opt_selectedBg=SettingsService.getBackground(),$scope.selectBackground=function(){$rootScope.themeBg=$scope.opt_selectedBg.class,SettingsService.setBackground($scope.opt_selectedBg),console.log("Changed background to '"+$scope.opt_selectedBg.name+"'")},$scope.toggleReversals=function(){$scope.opt_showReversals=!$scope.opt_showReversals,SettingsService.setAllowReversals($scope.opt_showReversals)},$scope.toggleMajorArcana=function(){confirm("Clear all cards and apply this setting?")&&($scope.opt_majorArcanaOnly=!$scope.opt_majorArcanaOnly,SettingsService.setMajorArcanaOnly($scope.opt_majorArcanaOnly),TarotService.setIsNewGame(!0))}}]);
"use strict";tarotifyAppDirectives.directive("appFooter",function($location){return{restrict:"E",templateUrl:"partials/app_footer.html",controller:function($scope,TarotService,LayoutManagerService){$scope.allPositionsFilled=!1,$scope.shuffleCards=function(){TarotService.shuffleCards()},$scope.drawCard=function(){var num_cards=TarotService.getCurrentCards().length,available_positions=LayoutManagerService.getTotalAvailablePositions();if(available_positions>0){var pick_card=_.random(0,num_cards-1);TarotService.drawCard(pick_card)}0===LayoutManagerService.getTotalAvailablePositions()&&($scope.allPositionsFilled=!0)},$scope.$on("NEW_READING",function(){$scope.allPositionsFilled=!1})},link:function(scope){scope.isHome=!0,scope.$on("$routeChangeSuccess",function(){scope.isHome="/"!==$location.path()?!1:!0})}}});
"use strict";tarotifyAppDirectives.directive("appHeader",function($location){return{restrict:"E",templateUrl:"partials/app_header.html",link:function(scope){scope.isHome=!0,scope.$on("$routeChangeSuccess",function(){scope.isHome="/"!==$location.path()?!1:!0})}}});
"use strict";tarotifyAppServices.factory("LayoutManagerService",function($http,$q){var _layout_data_loaded=!1,_layout_data=null,_original_layout=null,_current_layout=null,_d=$q.defer();return{initLayouts:function(){return $http({method:"GET",url:"config/layouts.json"}).success(function(data){_d.resolve(data),_layout_data=data.layout_data,_layout_data_loaded=!0}).error(function(data,status){_d.reject(data),console.log("ERROR: "+status+": layouts: ",data)}),_d.promise},setLayout:function(layoutName){return console.log("Getting layout data for '"+layoutName+"'"),_current_layout=_.filter(_layout_data,function(layout){return layout.name===layoutName})[0],_original_layout=angular.copy(_current_layout),_current_layout},setCardOnGridItem:function(card,nextNum){for(var num_grid_items=_current_layout.grid_items.length,i=0;num_grid_items>i;i++)_current_layout.grid_items[i].num===nextNum&&(_current_layout.grid_items[i].card=card)},resetCurrentLayout:function(){_current_layout=angular.copy(_original_layout)},getCurrentLayout:function(){return _current_layout},getAllLayouts:function(){return _layout_data},getTotalPositions:function(){for(var total_positions=0,num_grid_items=_current_layout.grid_items.length,i=0;num_grid_items>i;i++)_current_layout.grid_items[i].hasOwnProperty("num")&&total_positions++;return total_positions},getTotalAvailablePositions:function(){for(var available_positions=0,num_grid_items=_current_layout.grid_items.length,i=0;num_grid_items>i;i++)_current_layout.grid_items[i].hasOwnProperty("card")&&available_positions++;return this.getTotalPositions()-available_positions}}});
"use strict";tarotifyAppServices.factory("SettingsService",function(){var _background,_allow_reversals,_major_arcana_only,_backgrounds=[{name:"Wood 1","class":"wood1"},{name:"Wood 2","class":"wood2"},{name:"Wood 3","class":"wood3"},{name:"Dark Stone","class":"dark-stone"},{name:"Lost Blue","class":"lost-blue"}];if(_background=localStorage.getItem("opt_background"),_background=JSON.parse(_background),null!==_background)for(var i=0;i<_backgrounds.length;i++)_background.class===_backgrounds[i].class&&(_background=_backgrounds[i]);return _allow_reversals=localStorage.getItem("opt_allow_reversals"),_major_arcana_only=localStorage.getItem("opt_major_arcana_only"),null===_background&&(_background=_backgrounds[1]),null===_allow_reversals&&(_allow_reversals=1),null===_major_arcana_only&&(_major_arcana_only=0),{setBackground:function(bg){_background=bg,localStorage.setItem("opt_background",JSON.stringify(bg))},setAllowReversals:function(allowReversals){_allow_reversals=allowReversals?1:0,localStorage.setItem("opt_allow_reversals",_allow_reversals)},setMajorArcanaOnly:function(majorArcanaOnly){_major_arcana_only=majorArcanaOnly?1:0,localStorage.setItem("opt_major_arcana_only",_major_arcana_only)},getBackgrounds:function(){return _backgrounds},getBackground:function(){return _background},getAllowReversals:function(){return 1==_allow_reversals?!0:!1},getMajorArcanaOnly:function(){return 1==_major_arcana_only?!0:!1}}});
"use strict";tarotifyAppServices.factory("TarotService",function($http,$q,$rootScope,SettingsService){var _card_data_loaded=!1,_new_game=!0,_all_cards=[],_card_data=[],_d=$q.defer(),_preloadImages=function(){for(var i=0;i<_all_cards.length;i++)$http({method:"GET",url:_all_cards[i].image_url})};return{initCards:function(){return _card_data_loaded||$http({method:"GET",url:"config/card_data.json"}).success(function(data){_d.resolve(data),_all_cards=data.card_data,_card_data=_all_cards,_card_data_loaded=!0,_preloadImages()}).error(function(data,status){_d.reject(data),console.log("ERROR: "+status+": card_data: ",data)}),_d.promise},shuffleCards:function(){_card_data=_all_cards,SettingsService.getMajorArcanaOnly()&&(_card_data=_.filter(_card_data,function(card){return"Major Arcana"===card.class})),_card_data=_.shuffle(_card_data),$rootScope.$broadcast("CARDS_SHUFFLED")},addReversals:function(){for(var i=0;i<_card_data.length;i++){var is_reversed=Math.floor(2*Math.random());is_reversed=0===is_reversed?!1:!0,_card_data[i].reversed=is_reversed}},setIsNewGame:function(isNewGame){_new_game=isNewGame},drawCard:function(id){var card=_card_data[id];_card_data.splice(id,1),$rootScope.$broadcast("DREW_CARD",card)},isNewGame:function(){return _new_game},getCardByUrl:function(urlPath){for(var i=0;i<_all_cards.length;i++)if(_all_cards[i].url_path===urlPath)return _all_cards[i]},getCurrentCards:function(){return _card_data}}});